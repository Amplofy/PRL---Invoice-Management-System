...
const PO_DEFAULT_COLUMNS = ['po_serial', 'invoice_no', 'vendor', 't1', 't2', 'amount', 'released_amount', 'released_via', 'status']
...
export default function PaymentOrdersPage() {
  ...
  const col = useColumnVisibility('prl-eoms-cols-payment-orders-v4', PO_COLUMN_KEYS, PO_DEFAULT_COLUMNS)
  ...
  const filtered = useMemo(() => {
    ...
    const searched = q
      ? orders.filter((o) =>
          `${o.serial_no ?? ''} ${o.invoices?.invoice_no ?? ''} ${vendorOf(o)} ${contractOf(o)} ${o.invoices?.t1 ?? ''} ${o.invoices?.t2 ?? ''} ${o.invoices?.t3 ?? ''}`.toLowerCase().includes(q),
        )
      : orders
  ...
  const exportCSV = () =>
    downloadCSV(
      ...
      sorted.map((o) => ({
        ...
        ...(col.show('t1') ? { type: o.invoices?.t1 ?? '' } : {}),
        ...(col.show('t2') ? { service: o.invoices?.t2 ?? '' } : {}),
        ...(col.show('t3') ? { detail: o.invoices?.t3 ?? '' } : {}),
  ...
  const print = (order: PaymentOrder) => {
    ...
    const contractNo = contractNoOf(order)
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <HeaderTh columnKey="actions" className="col-actions" align="right" resizable={false}>Actions</HeaderTh>
              </tr>
            </thead>
            <tbody>
              {(() => {
