    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {admin && status !== 'Approved' && status !== 'Paid' && poVersions.length === 0 && (
            <button className="btn btn-danger" onClick={() => setDeleting(true)}>
              <Trash2 size={15} /> Delete
            </button>
          )}
          {admin && (status === 'Paid' || (status !== 'Approved' && poVersions.length > 0)) && (
          ...
          {status === 'Paid' && (
            <Link to="/payment-orders" className="btn btn-success">
              <Landmark size={15} /> Payment released
            </Link>
          )}
            ...
            <EmptyState
              ...
              description={
                status === 'Approved'
                  ...
                  : status === 'Paid'
                    ? 'Payment has been released. This invoice is closed.'
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th>Generated At</th>
                    <th>Status</th>
                    <th className="text-right">PO / Released</th>
                ...
                <tbody>
                  {poVersions.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>
                        <StatusBadge tone={poStatusTone(p.status)}>{poStatusLabel(p.status)}</StatusBadge>
                      </td>
                      <td className="text-right text-xs tabular-nums">
                        Rs {formatMoney(poGeneratedAmount(p, invoice))}
                        {poReleasedAmount(p) > 0 ? ` → ${formatMoney(poReleasedAmount(p))}` : ''}
                      </td>
                      <td>
                        <div className="flex justify-end">
                          <button className="btn btn-ghost btn-sm" title="View PO" onClick={() => setViewPo(p)}>
                            <FolderOpen size={14} />
                          </button>
                        </div>
                      </td>
        ...
        {viewPo && (
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <StatusBadge tone={poStatusTone(viewPo.status)}>{poStatusLabel(viewPo.status)}</StatusBadge>
            </div>
            <div className="flex justify-between border-t border-[var(--border)] pt-3">
              <span className="text-[var(--text-muted)]">Invoice approved</span>
              <b>Rs {formatMoney(invoiceApprovedAmount(invoice), 2)}</b>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">PO generated</span>
              <b>Rs {formatMoney(poGeneratedAmount(viewPo, invoice), 2)}</b>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">Payment released</span>
              <b>Rs {formatMoney(poReleasedAmount(viewPo), 2)}</b>
            </div>
